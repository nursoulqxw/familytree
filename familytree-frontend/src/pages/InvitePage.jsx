...
export default function InvitePage() {
  ...
  return (
    <div className="min-h-screen bg-cream flex items-center justify-center p-4">
      <div className="border border-cream-border rounded-2xl p-10 bg-cream-light shadow max-w-md w-full text-center">
        <div className="flex justify-center mb-4">
          <BrandMark size={48} />
        </div>
        <h1 className="font-serif font-black text-2xl text-ink mb-4">{t('invitePage.title')}</h1>

        {!accessToken ? (
          <>
            <p className="text-ink/70 text-sm">{t('invitePage.needLogin')}</p>
          ...
          <>
            ...
            >
              {submitting ? t('invitePage.joining') : t('invitePage.join')}
            </button>
          </>
        )}
      </div>
