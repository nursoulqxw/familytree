    ...
    <KeyboardAvoidingView style={{flex:1,backgroundColor:colors.cream,padding:24}} behavior={Platform.OS==='ios'?'padding':undefined}>
      <ScrollView contentContainerStyle={{flexGrow:1,justifyContent:'center'}} keyboardShouldPersistTaps="handled">
        <View style={{alignItems:'center',marginBottom:28}}>
          ...
          <Text style={{fontSize:22,fontFamily:'Times',fontWeight:'800',color:colors.ink, marginBottom:20}}>Родовое древо</Text>
          <Text style={{fontSize:17, fontFamily:'Times',color:colors.ink,opacity:0.6,marginTop:4,textAlign:'center'}}>Создайте аккаунт, чтобы начать своё дерево</Text>
        </View>
        <View style={{backgroundColor:colors.creamLight,borderRadius:radii.lg,borderWidth:1,borderColor:colors.creamBorder,padding:20}}>
          {[['Логин',username,setUsername,'none',false],['Email',email,setEmail,'none',false],['Пароль',password,setPassword,'sentences',true]].map(([lbl,val,setter,cap,sec]) => (
            <View key={lbl} style={{marginBottom:12}}>
              <Text style={{fontFamily:'Times',fontSize:12.5,fontWeight:'600',color:colors.ink,opacity:0.75,marginBottom:6}}>{lbl}</Text>
          ...
          <TouchableOpacity style={{backgroundColor:colors.olive,borderRadius:radii.sm,paddingVertical:13,alignItems:'center',marginTop:18}} onPress={handle} disabled={loading}>
            {loading ? <ActivityIndicator color={colors.white}/> : <Text style={{fontFamily:'Times',color:colors.white,fontWeight:'700',fontSize:15}}>Зарегистрироваться</Text>}
          </TouchableOpacity>
          <TouchableOpacity onPress={() => navigation.navigate('Login')} style={{marginTop:14,alignItems:'center'}}>
                    <Text style={{ fontFamily:'Times',fontSize: 13, color: colors.olive800 }}>Уже есть аккаунт?{' '}<Text style={{ fontWeight: '700' }}>Войти</Text></Text>
                  </TouchableOpacity>
        </View>
      </ScrollView>
